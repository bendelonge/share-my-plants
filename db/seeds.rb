...
avocatier = Plant.create!(species: 'Avocatier', city_location: 'Nantes', address: '58 boulevard Michelet', country: 'France', user: matt, price_per_day: 270, main_picture: File.open(Rails.root.join('db/fixtures/avocatier.jpg')), description: "Le plus diffcile dans sa culture est d'attendre que le noyau produise des racines! Mais, si vous l'achetez en pot, il poussera très rapidement tant qu'il sera bien abreuvé. Ses feuilles deviennent assez larges et longues(20 à 30 cm) et, selon la terre disponible, il peut rester à 2 m de haut ou faire le tour de la pièce où il est placé")
amaryllis = Plant.create!(species: 'Amaryllis', city_location: 'Paris', address: '15 rue Ginoux', country: 'France', user: ben, price_per_day: 145, main_picture: File.open(Rails.root.join('db/fixtures/amaryllis.jpg')), description: "Plante à bulbe, cet amaryllis d'intérieur produit une ou plusieurs tiges centralesportant d'énormes fleurs légèrement parfumées superbes en hiver")
gardenia = Plant.create!(species: 'Gardenia', city_location: 'Strasbourg', address: '25 avenue du Rhin', country: 'France', user: matt, price_per_day: 199, main_picture: File.open(Rails.root.join('db/fixtures/gardenia.jpg')), description: "Il est irrésistible, avec sa belle floraisonblnche et délicieusement parfumée, même s'il ne vit pas plus de 6 mois en appartement! En serre ou en véranda, il sera plus facile de lui offrir les variations de température adéquates.")
ficus = Plant.create!(species: 'Ficus pleureur', city_location: 'Londres', address: '20 oxford street', country: 'Royaume Uni', user: matt, price_per_day: 958, main_picture: File.open(Rails.root.join('db/fixtures/ficus.jpg')), description: "De feuillage dense, grâcieux et bien développé tout le long de ses nombreuses tiges, ce ficus croît rapidement pour former un grand arbuste et vit de longues années")
crassula = Plant.create!(species: 'Crassula', city_location: 'Versailles ', address: '60 avenue de Paris', country: 'France', user: ben, price_per_day: 129, main_picture: File.open(Rails.root.join('db/fixtures/crassula.jpg')), description: "Elle fait partie des plantes dites grasses dont les feuilles charnues retiennent l'eau dans leurs tissus. Le tronc, peu visible quand elle est jeune, est lui aussi remplé et donne à la plante l'aspect d'un petit arbre")
hypoestes = Plant.create!(species: 'Hypoestes', city_location: 'Paris', address: 'Avenue des champs Elysees', country: 'France', user: ben, price_per_day: 199, main_picture: File.open(Rails.root.join('db/fixtures/hypoestes.jpg')), description: "Tapissante et très colorée, cette petite plante a tout pour plaire quand on a peu de place. Vert tacheté de rose, de rouge ou de blanc, elle masque le pot de ses rameauxtouffus et retombants pour créer une boule de fueillage.")
scindapsus = Plant.create!(species: 'Scindapsus', city_location: 'Bruxelles',address: 'rue du damier', country: 'Belgique', user: matt, price_per_day: 59, main_picture: File.open(Rails.root.join('db/fixtures/scindapsus.jpg')), description: "Venue de Brésil, cette solanacée de la famille des piments porte ce joli nom en référence à ses baies rondes. Passant du vert au jaune puis au rouge brillant, cette parure prend toute sa valeur e Octbre et dure plusieurs semaines")
jasmin = Plant.create!(species: 'Jasmin de Madagascar', city_location: 'Nantes', address: '26 bd de Stalingrad', country: 'France', user: ben, price_per_day: 169, main_picture: File.open(Rails.root.join('db/fixtures/jasmin.jpg')), description: "C'est une liane robuste au feuillagecoriace et vernissé, peu abondant. De mai à Septembre, la floraison blanche en étoiles tubulaires est très odorante. Son parfum capiteux exhale ses effluves assez loin autour de la plante.")
abutilon = Plant.create!(species: 'Abutilon', city_location: 'Paris', address: 'gare montparnasse', country: 'France', user: ben, price_per_day: 109, main_picture: File.open(Rails.root.join('db/fixtures/abutilon.jpg')), description: "Arbustes semi-rustiques, les abutilons sont cultivés à l'extérieur dans les régions à climat doux, le plus souvent en plante saisonnière. Protégés du froid dans une maison ou une véranda, ils épanouissent du printemps à l'automne de grosses fleurs en clochettes sur un feuillage large et abondant.")
amayllis2 = Plant.create!(species: 'Amaryllis', city_location: 'Nantes',address: '166 boulevard des anglais', country: 'France', user: matt, price_per_day: 144, main_picture: File.open(Rails.root.join('db/fixtures/default_plant_picture.png')), description: "Plante à bulbe, cet amaryllis d'intérieur produit une ou plusieurs tiges centralesportant d'énormes fleurs légèrement parfumées superbes en hiver")
